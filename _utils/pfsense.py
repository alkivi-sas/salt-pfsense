...
class FauxapiLib:
    ...
    def get_singleton(
        ...
        SINGLETON_INSTANCE = cls(
            ...
            apikey=apikey,
            apisecret=apisecret,
            use_verified_https=use_verified_https,
            debug=debug,
        )
        return SINGLETON_INSTANCE
    ...
    def _get_apikey(self):
        ...
        raise CommandExecutionError(
            "Unable to extract PFFA key from {0}".format(credentials)
        )
    ...
    def _get_host(self, key):
        ...
        if not find_key:
            raise CommandExecutionError(
                "Unable to find key {1} in {0}".format(key, credentials)
            )
        else:
            return "127.0.0.1"
    ...
    def _get_apisecret(self, key):
        ...
        if not find_key:
            ...
            raise CommandExecutionError(
                "Unable to extract secret associated to key {1} in {0}".format(
                    key, credentials
                )
            )
    ...
    def config_get(self, section=None):
        ...
        raise FauxapiLibException(
            "unable to complete config_get request, section is unknown", section
        )
    ...
    def config_restore(self, config_file):
        return self._api_request(
            "GET", "config_restore", params={"config_file": config_file}
        )
    ...
    def alias_update_urltables(self, table=None):
        if table is not None:
            return self._api_request(
                "GET", "alias_update_urltables", params={"table": table}
            )
    ...
    def _api_request(self, method, action, params=None, data=None):
        ...
        if res.status_code == 404:
            raise FauxapiLibException(
                "Unable to find FauxAPI on target host, is it installed?"
            )
        elif res.status_code != 200:
            raise FauxapiLibException(
                "Unable to complete {}() request".format(action), json.loads(res.text)
            )
    ...
    def _generate_auth(self):
        ...
        hash = hashlib.sha256(
            "{}{}{}".format(self.apisecret, timestamp, nonce).encode("utf-8")
        ).hexdigest()
